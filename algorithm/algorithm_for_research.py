...
def optimize_for_user(backlog, user_df):
	user_experience_vector = user_df['vector']
...
def generate_solution_for_user():
    
	dataset = pd.read_csv('./dataset/jiradataset_issues_v1.4.csv', encoding='utf-8')

	dataset = pp.preprocess(dataset)

	dataset = dataset.loc[dataset["project"] == "xd"].iloc[:200]
	
	lda.get_user_experience_topic_vector2(dataset)
