...
def get_hypervolume(backlog, user_experience_vector, storypoints, generations):
	...
	hv_indicator = get_performance_indicator("hv", ref_point=np.array([1, 1, 1]))
...
def save_results():
	...
	for project, project_df in grouped_projects: 
		...
		for user, user_df in grouped_users: 
			...
			hypervolume = get_hypervolume(backlog, vector, 15)
...
def figure_out_convergence():
	...
	for project, project_df in grouped_projects: 
		...
		for user, user_df in grouped_users: 
			...
			plt.plot(list(range(1,generations+1)), hv,  color='black', lw=0.7, label="Avg. CV of Pop")
			plt.scatter(list(range(1,generations+1)), hv,  facecolor="none", edgecolor='black', marker="p")
			# plt.axvline(k, color="red", label="All Feasible", linestyle="--")

			plt.title("Convergence")
			plt.xlabel("Generations")
			plt.ylabel("Hypervolume")
			plt.savefig('./thesis-api/algorithm/convergence/' + project + '.png')
